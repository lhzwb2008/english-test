...
- `system_task_pool`（**生产必给**；亦称「系统任务库」）：业务方注入的原子任务清单。条目可含 `教材`、以及可选的 **`必须安排`** + **`安排顺序：N`**（见下）。**输出里 `days[].tasks[]` 必须全部从该清单挑选**，并把命中条目的 ID 原样回填到 `tasks[].sourceRef`。题库**不在** Bot System Prompt 里。未提供时所有 `sourceRef` 为 `""`（仅兜底）。
  - **必排任务**：条目含 `必须安排` 时，模型必须把这些 ID **全部**排进计划，并按 `安排顺序` 升序落在时间轴上（**同序必须同日、整组排完**；不得逆序）。必排优先于非必排；`priority` 填 `must`。
  - **每日名额**：从档案「可用学习时间」解析。`每天 N 个任务` / `每周一、三、五，每天 N 个任务` 中的 N 指**逻辑名额**：同一 `安排顺序` 下多条原子任务只占 **1** 个名额，故当天 `tasks.length` 可以大于 N。仅写 `每天2小时` 时按时长估算名额。
  - **池子耗尽时**：优先缩短 `days`，或对清单条目复用（同一 `sourceRef` 可重复，`detail_zh` 标注"复盘/巩固"），**绝不会**自拟池外任务。

**不传** `curriculum`：由模型从档案推断写入 `meta.curriculum`。若全文出现「使用教材：X」，模型**只排**池内 `教材` 与 X 匹配的条目，忽略其它教材（学习历史提到的旧教材、仅课程名撞关键字但 `教材` 字段不符的条目均不算）。任务库字段名兼容 `system_task_pool` / `系统任务库`。**不走 RAG**。建议业务侧注入前按使用教材预过滤。
...
- **`text` 可附带正确答案**：题号能对上时，`standard_answer` 与对错**以该标答为准**（不会用模型自己推断覆盖）。对不上或未提供的题，仍按无题库规则。
- **不传**教材全文、阅读 passage、作文评分量表：这些是**题库 / 知识库**侧职责。
- **当前为无题库版本**：未提供可匹配标答时，模型基于图片 OCR 出题干，能由通用语言知识独立确定的题目（语法填空、固定搭配等）会给出 `standard_answer`；阅读题若图中印有 passage 则结合 passage 判分；其它情况下 `standard_answer` 留空（`""`），并在 `reasoning_zh` / `limitations` 写明「因无题库未给标答」。
...
> 布置范围与标答都写在同一段 `text` 即可。标答按大题（`Exercise 4` / `第4题`）+ 小题编号给；能对上 `P48-4-1` 这类题号时，批改以该答案为准。只给了第 4 题标答、布置里还有第 5 题时，第 5 题仍按无题库规则。不传标答时行为与原来一致。
