...
5. **对不上就不用**：格式残缺、大题号对不上、或无法判断是哪一空 → 该空当无标答，并在 `limitations` 写「标答未匹配：…」。禁止猜着套。
...
6. 可在 `limitations` 写一句「已采用调用方提供的标答：P48-4」。
...
- **`standard_answer`**：
  ...
  - **未匹配到调用方标答**时：题目能由**通用英语语言知识**单独确定（如『My brother ___ football every weekend.』→ `plays`、固定搭配等）可以填入；否则**留空**（`""`），并在 `reasoning_zh` 中明示『因无题库，未给出标答』。
