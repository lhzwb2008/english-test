...
也兼容：`第4题` / `大题4` / `Ex.4` / `4.` 作为大题头；小题 `1. B` / `1) B` / `1 B` / `1、went`；填空/短答写单词或短语。
...
| 标答写法 | 对齐到 |
|---|---|
| `Exercise 4` / `Ex.4` / `第4题` / `大题4` 下的 `1、B` | `P{页}-{4}-{1}`，`standard_answer=B` |
| 仅有大题、无小题编号（整题一个答案） | `P{页}-{大题号}` |
| 页码优先用 `text` 布置范围里的 `P48`；标答自己写了页码则用标答页码 | 不得把 P48 的第4题对到别的页 |

匹配规则：

1. **大题号必须对上**：`Exercise 4` 只约束第 4 大题，不能套到第 5 题。
2. **小题号必须对上**：标答 `3、A` 只给 `P48-4-3`（或印刷题号就是 3 的那一空），禁止错行（把第 1 空的 B 写到第 2 空）。
3. **只覆盖能对上的题**：布置了第 4–5 题但标答只有 Exercise 4 → 第 4 题用标答；第 5 题仍按无题库规则（能独立确定就给标答，否则 `""`）。
4. **标答多出来的题号**：若超出必做/选做范围，**不要**为了用标答而多输出 `items[]`。
5. **对不上就不用**：格式残缺、大题号对不上、或无法判断是哪一空 → 该空当无标答。禁止猜着套。若因此只能按题目内容判断，`limitations` 可写「部分小题没有对应答案，已按题目内容判断」，禁止写「未匹配 / 调用方」。

## 2. 命中之后怎么判（硬约束）

对该空/题一旦匹配成功：

1. **`standard_answer` 必须写成调用方给出的答案**（选项题保留字母 `A`/`B`/`C`/`D`；填空保留其单词/短语）。禁止改成你认为「语言上更对」的另一个选项。
2. **先按圈选痕迹锁定 `student_answer`，再与该标答比较**（顺序不可颠倒）。禁止因为标答是 B 就把模糊圈读成 B。
3. **`is_correct` 只比较学生答案与调用方标答**（先做答案归一化）。相等/等价 → 必须 `true`；不等 → `false`。不得因为「原文/语法上另一个选项更好」而改判。
4. **`explanation_zh` / `reasoning_zh` 必须以该标答为正确结论**。可以讲为什么是这个答案，但禁止输出「标答写 B，但其实应选 D」来否定调用方标答。
5. **不得留空**：命中的题禁止 `standard_answer=""`，禁止写「因无题库未给标答」。
6. **禁止把内部判分过程写进给学生看的字段**（见下一节）。不要在 `reasoning_zh` / `explanation_zh` 里写「调用方」「标签」「匹配标答」等。
