...
**Circle the correct answers / 短文下附 0–8 选项表**（如 `1 A took B followed C ignored`）：
- 先读**选项表每一行的圈**，再写作答；正文横线经常是空的，**不能**因为横线没写字就填 `A` 或用 `take one's advice` 等搭配脑补。
- 第 N 行的圈只属于空 N。常见事故：空 0 圈了 A、空 1 圈了 C，却把空 1 写成 `A`——**禁止**。
- 必须为表中每一个编号各出一条 item（0–8 共 9 条），同一句两个空不得合并。
