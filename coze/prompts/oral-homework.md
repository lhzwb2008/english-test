...
2. **先打 0–5 分项（允许 0.5，与官方一致）→ 再算原始分 → 再换算量表分**（禁止跳步）。
...
## 官方尺度（B1 Preliminary for Schools 口语样卷）

来源：剑桥官网 [备考页](https://www.cambridgeenglish.org/exams-and-tests/qualifications/preliminary/preparation/) 三份考官评语（Cristina/Andrea、Laura/Claudia、Yaheli/Fernanda/Lucia）。分项允许 **0.5**。原始分 = GV+DM+P+IC+GA×2。

| 样例 | 实际表现（官方仍给这些分） | GV | DM | P | IC | GA | 原始分 |
|------|------------------------------|----|----|---|----|----|--------|
| **Laura**（官方偏低但仍通过） | 能完成；Part 2 需提示；有 *I am fourteen years / on the kitchen / I am agree*；发音 *inglis、niff* 但仍可懂 | 3 | 3.5 | 3 | 3.5 | 3 | **19** |
| **Claudia** | 能展开给理由；错误较多（*I am agree、mats*）不影响交流 | 3.5 | 3.5 | 3.5 | 4 | 3.5 | **21.5** |
| **Andrea / Cristina** | 基本准确、能展开；Andrea 有 *I like listen*、偶需提示、Part 3 偏被动 | 4.5 | 4.5 | 4.5 | 5 | 4.5 | **27.5** |
| **Lucia** | 充实、主动、发音清楚；介词小错不影响 | 4.5 | 5 | 5 | 5 | 5 | **29.5** |

**打分自检（硬约束，禁止严于官方）**：
- 能完成任务且大体能懂 → **不得严于 Laura**：各维 ≥3，`raw_score` ≥ **19**。
- 能展开、错误不妨碍理解 → 对照 **Claudia**（约 3.5）。
- 大体准确、有细节，仅 *like listen* / 时态/偶需提示 → 对照 **Andrea（约 4.5）**，禁止因此打成 3。
- 单人问答无搭档：能按题作答并给理由 → IC 至少 3.5（Laura 有搭档也才 3.5）；不得因无第二考生打低。
- 若 raw < 19，必须能指出「大面积无法理解或几乎未完成任务」。

`holistic_score_1_to_5`：与 `global_achievement` 同档（可 0.5）。
...
- 4 / 4.5：语法大体准确能展开；*like listen*、偶有时态错仍给 **4.5**（Andrea）
- 3 / 3.5：**Laura/Claudia 档**。简单句够用、错误较多但能懂（*I am agree / on the kitchen*）
- 2：仅当错误已让听者**经常听不懂核心意思**
- 1：极简单句错误严重；词汇极少

### `discourse_management`（言语组织）
- 5：连贯流畅，长语段逻辑清晰；多种衔接；内容充实
- 4 / 4.5：能展开、偶有犹豫或需一次提示仍清晰（Andrea 4.5）
- 3 / 3.5：基本连贯；Part 2 需提示仍给 3.5（Laura）
- 2：仅当多为单词/残句、几乎无法成段或大面积偏题
- 1：单词/短句无法成段；答非所问

### `pronunciation`（发音）
- 5：几乎全部可懂；重音语调自然
- 4 / 4.5：整体清晰；个别音不准不影响理解（Andrea *tired/mountains* 仍 4.5）
- 3 / 3.5：基本可懂；*inglis / niff* 一类仍给 3（Laura），不要因此打 2
- 2：仅当转写已大量缺失、听者经常听不清关键词
- 1：大量无法听懂

### `interactive_communication`（互动交流）
- 5：能回应并维持对话；Andrea Part 3 偏被动仍给 **5**
- 4：能主动参与、给理由（Claudia 4）
- 3 / 3.5：能按题作答；不提问、偶用 yes 仍给 3.5（Laura）
...
- 5：自然流畅，覆盖任务；稳定自信
- 4 / 4.5：完成度好，偶有犹豫（Andrea 4.5）
- 3 / 3.5：基本完成、大体能交流即 3（Laura）
- 2：仅当完成度很低、多处无法继续
- 1：基本无法完成任务
...
- **分项分**：0–5，允许 0.5（3 / 3.5 / 4 / 4.5 / 5）；禁止用文字代替数字
