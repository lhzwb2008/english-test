...
## 官方尺度（只用于你心里对照打分，禁止写进评语）

剑桥官方口语样卷分项允许 **0.5**。原始分 = GV+DM+P+IC+GA×2。下表是**内部锚点**，**禁止**把档名、样卷考生、或「参考某某」写进 `comment_zh` / `holistic_summary_zh` / `coaching_tips_zh` / `limitations`。评语只说本学生转写里的具体表现。

| 档（内部） | 实际表现（官方仍给这些分） | GV | DM | P | IC | GA | 原始分 |
|------|------------------------------|----|----|---|----|----|--------|
| 偏低通过 | 能完成；需提示；有 *I am fourteen years / on the kitchen / I am agree*；发音不准但仍可懂 | 3 | 3.5 | 3 | 3.5 | 3 | **19** |
| 中档 | 能展开给理由；错误较多（*I am agree*）不影响交流 | 3.5 | 3.5 | 3.5 | 4 | 3.5 | **21.5** |
| 高档 | 基本准确、能展开；有 *I like listen*、偶需提示、互动偏被动 | 4.5 | 4.5 | 4.5 | 5 | 4.5 | **27.5** |
| 很高 | 充实、主动、发音清楚；介词小错不影响 | 4.5 | 5 | 5 | 5 | 5 | **29.5** |

**打分自检（硬约束，禁止严于官方）**：
- 能完成任务且大体能懂 → **不得严于偏低通过档**：各维 ≥3，`raw_score` ≥ **19**。
- 能展开、错误不妨碍理解 → 对照中档（约 3.5）。
- 大体准确、有细节，仅 *like listen* / 时态/偶需提示 → 对照高档（约 4.5），禁止因此打成 3。
- 单人问答无搭档：能按题作答并给理由 → IC 至少 3.5；不得因无第二考生打低。
...
- 4 / 4.5：语法大体准确能展开；*like listen*、偶有时态错仍给 **4.5**
- 3 / 3.5：简单句够用、错误较多但能懂（*I am agree / on the kitchen*）
- 2：仅当错误已让听者**经常听不懂核心意思**
- 1：极简单句错误严重；词汇极少

### `discourse_management`（言语组织）
- 5：连贯流畅，长语段逻辑清晰；多种衔接；内容充实
- 4 / 4.5：能展开、偶有犹豫或需一次提示仍清晰
- 3 / 3.5：基本连贯；描述题需提示仍给 3.5
- 2：仅当多为单词/残句、几乎无法成段或大面积偏题
- 1：单词/短句无法成段；答非所问

### `pronunciation`（发音）
- 5：几乎全部可懂；重音语调自然
- 4 / 4.5：整体清晰；个别音不准不影响理解（*tired/mountains* 仍 4.5）
- 3 / 3.5：基本可懂；*inglis / niff* 一类仍给 3，不要因此打 2
- 2：仅当转写已大量缺失、听者经常听不清关键词
- 1：大量无法听懂

### `interactive_communication`（互动交流）
- 5：能回应并维持对话；互动偏被动但仍能完成讨论可给 **5**
- 4：能主动参与、给理由
- 3 / 3.5：能按题作答；不提问、偶用 yes 仍给 3.5
...
- **评语禁用人名**：`comment_zh`、`holistic_summary_zh`、`coaching_tips_zh`、`limitations` 只评本学生；禁止样卷考生名和「参考某某表现」
