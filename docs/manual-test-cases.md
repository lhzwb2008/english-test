...
目标：小学三年级暑假 KET 达到卓越；小学五年级暑假 PET 达到优秀。请从当前进度起排两周课。

请按人设仅输出学习计划 JSON。
```

**预期**：可解析 JSON；含 `meta`（含 `curriculum`、`assumptions`）与 `days[]`（`day_index` 递增，无公历 `date`）；`lesson_code` 为 Prompt 内置 think1 库中 **`####` 标题原文**，非自拟代号；说明文字为**简体中文**。
