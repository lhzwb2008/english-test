...
目标成片 **60–90 秒**。老师反馈样片「还可以，但节奏要再快一点」——宁可短而密，不要拖。
...
- trap.wrong.lines / trap.right.lines 用题里的原句，不要改写英文
...
`narration.voice` 只允许 `zh` 或 `en`。

输出前自检：有 intro / trap / answer / ending；无寒暄；英文不在中文段里；中文总字数不超过 260。
